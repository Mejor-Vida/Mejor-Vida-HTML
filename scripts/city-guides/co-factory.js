/**
 * Shared Colorado city-guide builder. Same locked layout as Lincoln.
 ...
 * estimator column is Colorado Funeralocity averages (captured 26 Jul 2026).
 */
const CO_AVG = {
  directCremation: 1730,
  immediateBurial: 4864,
  memorialCremation: 5840,
  traditional: 8162,
};
...
const AVG_NOTE = {
  dc: {
    es: "Promedio de Colorado (Funeralocity), el mismo que usa el estimador.",
    en: "Colorado average (Funeralocity), the same figure the estimator uses.",
  },
  ib: {
    es: "Promedio de Colorado (Funeralocity); suele incluir un ataúd básico.",
    en: "Colorado average (Funeralocity); often includes a basic casket.",
  },
  mem: {
    es: "Cremación completa de Colorado (Funeralocity); suele incluir velatorio y ataúd de cremación.",
    en: "Colorado full cremation (Funeralocity); typically viewing and a cremation casket.",
  },
  tr: {
    es: "Entierro completo de Colorado (Funeralocity); ya mete un ataúd típico.",
    en: "Colorado full burial (Funeralocity); already puts in a typical casket.",
  },
};
...
function makeCity(spec) {
  ...
  const packages = [
    line("Cremación directa", "Direct cremation", PKG_DC_ES, PKG_DC_EN, {
      ...
      us: cell(CO_AVG.directCremation, AVG_NOTE.dc.es, AVG_NOTE.dc.en),
    }),
    line("Entierro inmediato", "Immediate burial", PKG_IB_ES, PKG_IB_EN, {
      ...
      us: cell(CO_AVG.immediateBurial, AVG_NOTE.ib.es, AVG_NOTE.ib.en),
    }),
    line("Cremación con memorial", "Cremation with memorial", PKG_MEM_ES, PKG_MEM_EN, {
      ...namedHomeCells(homes, (h) => h.memCell || NOT_PUB),
      us: cell(CO_AVG.memorialCremation, AVG_NOTE.mem.es, AVG_NOTE.mem.en),
    }),
    line("Funeral tradicional con velatorio", "Traditional funeral with visitation", PKG_TR_ES, PKG_TR_EN, {
      ...
      us: cell(CO_AVG.traditional, AVG_NOTE.tr.es, AVG_NOTE.tr.en),
    }),
  ];
  ...
  const homeLabels = {
    us: { en: "Estimator (Colorado)", es: "Estimador (Colorado)" },
  };
  for (const h of homes) {
    const row = {};
  ...
  return {
    ...
    descEs: `Seguro de gastos finales, de entierro o funeral en ${nameEs}${nearEs ? `, ${nearEs}` : ""}: funerarias, lotes y cotización por teléfono. Licencia de Colorado, NPN #21695431.`,
    descEn: `Final expense, burial, or funeral insurance in ${name}${nearEn ? `, ${nearEn}` : ""}: funeral-home lists, plots, and a phone quote. Colorado license, NPN #21695431.`,
    ctaSubEs:
    ...
    metroNoteEs: `Las cotizaciones oficiales son por teléfono para residentes de Colorado en estas comunidades${
      spec.countyEs ? ` del ${spec.countyEs}` : ""
    }. No hay oficina de atención al público. ${ownGuideLinks("es", others)}`,
    metroNoteEn: `Official quotes are by phone for Colorado residents in these${
        ...
        (dcs.length
          ? `Con cremación directa publicada desde ${joinListEs(
              ...
              trads[0]
                ...
                : "el promedio de Colorado es $8,162"
            }, y el lote, la bóveda y la lápida van aparte.`
          : homes.length
            ? `Esta ciudad tiene funerarias en la tabla, pero no todas publican cremación directa. El estimador usa $1,730 como promedio de Colorado para cremación directa; ese número no es el precio de una casa. Muchas familias eligen $5,000 a $10,000 para el servicio, urna, viajes y cuentas pequeñas. Un entierro tradicional suele necesitar más: el promedio de Colorado es $8,162, y el lote, la bóveda y la lápida van aparte.`
            : `Ninguna funeraria de ${nameEs} publica una lista general de precios en internet. El estimador usa $1,730 como promedio de Colorado para cremación directa; ese número no es el precio de una casa. Muchas familias eligen $5,000 a $10,000 para el servicio, urna, viajes y cuentas pequeñas. Un entierro tradicional suele necesitar más: el promedio de Colorado es $8,162, y el lote, la bóveda y la lápida van aparte.`),
    },
    faqCremationEn: {
        ...
        (dcs.length
          ? `With direct cremation published from ${joinListEn(
              ...
              trads[0]
                ...
                : "the Colorado average is $8,162"
            }, and the plot, vault, and marker are extra.`
          : homes.length
            ? `This city has funeral homes on the chart, but not every home publishes direct cremation. The estimator uses $1,730 as the Colorado average for direct cremation; that figure is not one home’s price. Many families choose $5,000 to $10,000 for the service, urn, travel, and small bills. A traditional burial often needs more: the Colorado average is $8,162, and the plot, vault, and marker are extra.`
    ...
    guide: {
      ...
      analysisEs: [
        {
          ...
          p:
            ...
            (dcs.length
              ? `La cremación directa es el atajo más barato que publican: ${dcBitsEs}. El estimador usa <strong>$1,730</strong> para Colorado. Ese número es solo gastos de funeraria: urna, flores y certificados van aparte.`
              : homes.length
                ? `Las casas de esta tabla no publican cremación directa. El estimador usa <strong>$1,730</strong> como promedio de Colorado. Ese número no es el precio de una casa. Pida la lista vigente por teléfono. Urna, flores y certificados van aparte.`
                : `Ninguna funeraria de ${nameEs} publica una lista general de precios en internet, así que esta tabla no las nombra. El estimador usa <strong>$1,730</strong> como promedio de Colorado para cremación directa. Ese número no es el precio de una casa. Pida la lista vigente por teléfono. Urna, flores y certificados van aparte.`),
        },
        {
      ...
      analysisEn: [
        {
          ...
          p:
            ...
            (dcs.length
              ? `Direct cremation is the cheapest published shortcut: ${dcBitsEn}. The estimator uses <strong>$1,730</strong> for Colorado. That number is funeral home expenses only: urn, flowers, and death certificates are extra.`
              : homes.length
                ? `The homes on this chart do not publish direct cremation. The estimator uses <strong>$1,730</strong> as the Colorado average. That figure is not one home’s price. Ask for the current list by phone. Urn, flowers, and death certificates are extra.`
                : `No funeral home in ${name} publishes a general price list online, so this table does not name them. The estimator uses <strong>$1,730</strong> as the Colorado average for direct cremation. That figure is not one home’s price. Ask for the current list by phone. Urn, flowers, and death certificates are extra.`),
        },
        {
      ...
      homes: [
        ...
        })),
        {
          id: "us",
          estimator: true,
          nameEs: "Estimador (Colorado)",
          nameEn: "Estimator (Colorado)",
          addrEs: "Promedios Funeralocity de Colorado",
          addrEn: "Colorado Funeralocity averages",
        },
      ],
      packages,
...
const FRONT_RANGE_NEAR = [
  { slug: "denver", name: "Denver" },
  { slug: "aurora", name: "Aurora" },
];

function frontRangeNear(except) {
  return FRONT_RANGE_NEAR.filter((x) => x.slug !== except);
}

module.exports = { CO_AVG, makeCity, cell, frontRangeNear };
