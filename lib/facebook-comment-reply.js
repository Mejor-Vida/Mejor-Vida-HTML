 ...
 *
 * Locked style (do not drift toward website-chat copy):
 * - First reply to a person: thank them; say you are the automated assistant of
 *   Mejor Vida Seguros / Mejor Vida Insurance; Julie was notified and will send
 *   a private message ASAP; then one short answer if they asked a question.
 * - Later replies: skip that intro. One short answer. End with
 *   “Si tiene otra pregunta, puede escribirla aquí.”
 * - Spanish usted. No URLs, phones, emails, licensed-state roster, carrier lists,
 *   sample price tables, or “call Julie.” Bare INFO / Information = intro only.
 * Practiced answers live in lockedFacebookAnswer() — do not send whoIsJulieAnswer
 * or ageCarrierFitAnswer from rag-pipeline.js into a public comment.
 */
...
function wrapFacebookReply(text, answer, opts) {
  ...
  if (first) {
    ...
    return `${commentOpening(text, { askForQuestion: false })}\n\n${body}\n\n${followUpLine(text)}`;
  }
  if (!body) {
...
function lockFacebookAnswer(answer) {
  ...
  t = t.replace(
    /\b(Assurity|Mutual of Omaha|Transamerica|American Amicable|Aetna|Accendo|Corebridge|Protect\+|Perform\+|Living Promise)\b[^.]{0,40}/gi,
    ""
  );
  t = t.replace(/\s+[–—-]\s*$/g, "");
  t = t.replace(/\s{2,}/g, " ").replace(/\n{2,}/g, " ").trim();
  const parts = t.split(/(?<=[.!?])\s+/).filter((p) => p && /[A-Za-zÁÉÍÓÚáéíóúñÑ]/.test(p));
  return parts.slice(0, 2).join(" ").trim().slice(0, 320);
}
  ...
  const out = await runRagPipeline(
    {
      ...
      conversationContext:
        `Public Facebook comment. Automated assistant. Julie will private-message them. ` +
        `ONE short answer, 1–2 sentences, Spanish usted. No URLs, no phone, no email, no carrier list, ` +
        `no licensed-state roster, no sample prices, no “call Julie.”`,
    },
    { hubspotNotePrefix: "Facebook comment RAG" }
  );
  if (out && out.answer) return lockFacebookAnswer(out.answer);
  return "";
}

async function composeReply({ intent, message, firstInConversation }) {
  const first = firstInConversation !== false;
  const wrapOpts = { firstInConversation: first };
  if (
    isInfoOnly(message) ||
    ...
    if (first) return commentOpening(message, { askForQuestion: true });
  ...
  return wrapFacebookReply(message, answer, wrapOpts);
}
