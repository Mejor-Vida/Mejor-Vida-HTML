/**
 * Phone SMS verification — normalize US numbers, OTP hash, Telnyx send, Supabase storage.
 */
...
async function sendOtpSms(toE164, body) {
  return sendSms({ to: toE164, body });
}
...
async function createAndSendOtp(supabaseUrl, key, phoneE164) {
  ...
  const sent = await sendOtpSms(phoneE164, smsBody);
  if (!sent.ok) {
