...
function renderGuideMediaCardInner(g, options, extra = {}) {
  ...
  const webp = `${imgPrefix}${g.image}.webp?v=20260911s`;
  const jpg = `${imgPrefix}${g.image}.jpg?v=20260911s`;
  ...
  return (
    ...
    `<source type="image/webp" srcset="${escHtml(webp)}"/>` +
          `<img src="${escHtml(jpg)}" alt="${escHtml(question)}" width="1200" height="900" ${imgAttrs}/>` +
    `</picture>` +
    `</div>` +
