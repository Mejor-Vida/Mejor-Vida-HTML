...
/** Tobacco users are quoted on a shorter ladder — preferred tobacco is the best real class. */
const LOW_CLASSES_TOBACCO = ["preferred_t"];
...
function lowClassesForProduct(product, smoker, maxBuildClass) {
  const simplified = product === "term_life_express" || product === "easy_term";
  let base;
  if (smoker) {
    base = simplified ? SIMPLIFIED_LOW_TOBACCO : LOW_CLASSES_TOBACCO;
  } else {
    base = simplified ? SIMPLIFIED_LOW_NT : LOW_CLASSES;
  }
