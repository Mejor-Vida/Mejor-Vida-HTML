...
async function exchangeLongLivedUserToken(shortLivedUserToken) {
  ...
  const accessToken = String(json.access_token || "").trim();
  if (!accessToken) throw new Error("Long-lived exchange returned no access_token");
  return { access_token: accessToken, tokenType: "long_lived_user" };
}
...
async function createPermanentMetaAdToken(adminUserToken) {
  ...
  try {
    ...
    return { ...result, tokenType: "system_user", expiring: false };
  } catch (err) {
...
/** System user token first; long-lived user token (~60 days) if Meta blocks system user API. */
async function createMetaAdAccessToken(userToken, options) {
  const userOnly = !!(options && options.userOnly);

  if (!userOnly) {
    try {
      return await createPermanentMetaAdToken(userToken);
    } catch (err) {
      const msg = String(err.message || "");
      const permissionBlocked =
        /permission|not have permission|login review|approved/i.test(msg);
      if (!permissionBlocked) throw err;
      const longLived = await exchangeLongLivedUserToken(userToken);
      return {
        ...longLived,
        fallbackReason:
          "System user token blocked (" +
          msg +
          "). Using long-lived user token (~60 days). Re-run /api/staff/meta-ads-auth to renew.",
      };
    }
  }

  return exchangeLongLivedUserToken(userToken);
}
