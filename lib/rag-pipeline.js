...
function whoIsJulieAnswer(isSpanish) {
  if (isSpanish) {
    return (
      "Julie Braunsroth es la fundadora de Mejor Vida Insurance y una agente de seguros licenciada en Nebraska, Kansas, Colorado y Nevada (NPN #21695431). " +
...
function applyKnowledgeAnswerPostProcess(question, answerText, isSpanishQuery, skipFaqHealth) {
  ...
  if (isLocationQuestion(question) && out) {
    ...
    if (!(t.includes("16820 frances") && t.includes("omaha"))) {
      out = isSpanishQuery
        ? "Mejor Vida Insurance está ubicada en Lincoln, Nebraska. También puedes contactar a Julie por llamada, texto o WhatsApp al 402-440-5438. Atendemos a clientes en Nebraska, Kansas, Colorado y Nevada."
  ...
  if (isContactPhoneQuestion(question) && out) {
    ...
    if (!hasAllThree) {
      out +=
        isSpanishQuery
          ? ' También guarda el 402-207-8568 como línea de oficina de Mejor Vida Insurance y el 402-440-5438 como línea directa de Julie. El +1 (402) 844-1199 es para mensajes de texto automáticos.'
