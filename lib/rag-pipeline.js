...
/** Avoid caching high-variance contact/quote/location prompts that can pollute FAQ hits. */
function shouldSkipFaqCaching(question) {
  const t = String(question || "").toLowerCase();
  return /\b(phone|whatsapp|contact|reach|quote|located|address|tel[eé]fono|contacto|cotizaci[oó]n|ubicad|direcci[oó]n|compa[ñn][ií]as|carriers?)\b/i.test(
    t,
  );
}
...
function shouldSkipFaqLookup(question) {
  return shouldSkipFaqCaching(question);
}
...
async function runRagPipeline(body, opts) {
  ...
  let faqMatch;
  if (!skipFaqHealth && !shouldSkipFaqLookup(question)) {
    try {
  ...
  if (!skipFaqHealth && !shouldSkipFaqCaching(question)) {
    try {
