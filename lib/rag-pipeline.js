...
function isAgentOnlyQuestion(question) {
  const t = String(question || "")
    ...
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");
  return /\b(commission|comision|comisiones|overrides?|agent (rates?|portal|only)|producer (guide|only)|drug list|lista de (drogas|medicamentos)|underwriting (chart|grid|manual)|milliman|intelliscript|quote\s*&\s*enroll)\b/.test(
    t,
  );
}
...
function isLocationQuestion(question) {
  ...
  const t = String(question || "")
    ...
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "");
  return /(where.*located|where are.*located|ubicad|direccion(?! de casa)|address(?! of julie'?s home)|oficina)/.test(t);
}
...
function applyKnowledgeAnswerPostProcess(question, answerText, isSpanishQuery, skipFaqHealth) {
  ...
  if (isLocationQuestion(question) && out) {
    ...
    if (!(t.includes("16820 frances") && t.includes("omaha"))) {
      out = isSpanishQuery
        ? "Mejor Vida Seguros está ubicada en Lincoln, Nebraska. También puedes contactar a Julie por llamada, texto o WhatsApp al 402-440-5438. Atendemos a clientes en Nebraska, Kansas, Colorado y Nevada."
        : "Mejor Vida Insurance is based in Lincoln, Nebraska. You can also contact Julie by call, text, or WhatsApp at 402-440-5438. We serve clients in Nebraska, Kansas, Colorado, and Nevada.";
    }
  }
  if (isContactPhoneQuestion(question) && out) {
    const hasAllThree =
      out.includes("402-440-5438") &&
      out.includes("402-207-8568") &&
      (out.includes("844-1199") || out.includes("(402) 844-1199"));
    if (!hasAllThree) {
      out +=
        isSpanishQuery
          ? ' También guarda el 402-207-8568 como línea de oficina de Mejor Vida Seguros y el 402-440-5438 como línea directa de Julie. El +1 (402) 844-1199 es para mensajes de texto automáticos.'
          : ' Also save 402-207-8568 as the Mejor Vida Insurance office line and 402-440-5438 as Julie\'s direct line. +1 (402) 844-1199 is for automated text updates.';
    }
  }
  return out;
}
