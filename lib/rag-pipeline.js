...
function applyKnowledgeAnswerPostProcess(question, answerText, isSpanishQuery, skipFaqHealth, language) {
  let out = answerText;
  if (skipFaqHealth && out) {
    ...
    if (looksLikeEligibilityPromise) {
      out = healthEligibilityDeferralLine(language);
    }
  }
...
async function runRagPipeline(body, opts) {
    ...
    if (isSpanishQuery && chunks && chunks.length > 0) {
      ...
      if (
        ...
        if (directAnswer && !/^NO_ANSWER$/i.test(directAnswer)) {
          ...
          let answerText = applyKnowledgeAnswerPostProcess(
            question,
            directAnswer,
            isSpanishQuery,
            skipFaqHealth,
  ...
  answerText = applyKnowledgeAnswerPostProcess(question, answerText, isSpanishQuery, skipFaqHealth, language);
