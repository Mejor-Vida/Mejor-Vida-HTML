...
async function runRagPipeline(body, opts) {
  ...
  const skipWebsiteCanned = isFacebookComment;
  ...
  if (!skipWebsiteCanned) {
    ...
    if (funeralAnswer) {
      usageBase.outcome = "funeral_gpl";
      logRagUsage(usageBase);
      return { answer: funeralAnswer, status: "answered", usage: usageBase };
    }
  }
  ...
  if (!skipFaqHealth && !skipFaqForSpanish && !shouldSkipFaqCachingQuestion(question) && !isFacebookComment) {
    try {
