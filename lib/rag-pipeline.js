...
function ageCarrierFitAnswer(age, isSpanish) {
  ...
  if (isSpanish) {
    if (age >= 86 && age <= 89 && aetnaOk) {
      return `A los ${age} años, entre los productos públicos que Julie cotiza, las edades de emisión suelen llegar más alto con Aetna: Accendo Final Expense (Nivelado hasta ~89) y Protection Series℠ Final Expense (hasta ~89). Varias otras líneas (Mutual of Omaha Living Promise, American Amicable, Assurity Protect+/Perform+, Transamerica FE Express, Corebridge SimpliNow/GIWL) suelen topar cerca de 80–85. Esto es solo por edad publicada — la salud y el estado también importan. Julie confirma qué aplica en su caso: 402-440-5438.`;
    }
    if (age > 89) {
      return `A los ${age} años, la mayoría de los productos de gastos finales que Julie cotiza públicamente ya no aceptan nuevas solicitudes (muchos topan a 85 o 89). Puede haber excepciones o referencias según el caso. Lo mejor es hablar con Julie al 402-440-5438 para revisar opciones reales.`;
    }
    const bits = [];
    ...
    return `A los ${age} años, Julie suele comparar aseguradoras cuyas edades de emisión publicadas incluyen esa edad — por ejemplo: ${list}. La “mejor” compañía depende también de salud, presupuesto y estado, no solo de la edad. Julie le orienta sin presión: 402-440-5438 o la cotización gratis.`;
  }

  if (age >= 86 && age <= 89 && aetnaOk) {
    return `At age ${age}, among products Julie quotes publicly, published issue ages usually go highest with Aetna: Accendo Final Expense (Level through about 89) and Protection Series℠ Final Expense (through about 89). Several other lines (Mutual of Omaha Living Promise, American Amicable, Assurity Protect+/Perform+, Transamerica FE Express, Corebridge SimpliNow/GIWL) often top out near 80–85. This is age-band guidance only — health and state also matter. Julie confirms what applies for you: 402-440-5438.`;
  }
  if (age > 89) {
    return `At age ${age}, most final expense products Julie quotes publicly no longer take new applications (many stop at 85 or 89). There may be case-by-case options or referrals. Best next step is Julie at 402-440-5438.`;
  }
  const bits = [];
  ...
  return `At age ${age}, Julie usually compares carriers whose published issue ages include that age — for example: ${list}. The “best” company also depends on health, budget, and state, not age alone. Julie can guide you with no pressure: 402-440-5438 or the free quote tool.`;
}
...
function isCarriersQuestion(question) {
  ...
  const asksRelationship = /\b(work|works|working|use|uses|offer|offers|tienen|tiene|trabaja|trabajan|trabajar|compar|compare|quote|cotiz)\b/.test(
    t
  );
