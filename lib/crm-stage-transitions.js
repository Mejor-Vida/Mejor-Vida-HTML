...
function stateFromProfile(pd) {
  if (!pd || typeof pd !== "object") return "";
  const ext = pd.profile_ext && typeof pd.profile_ext === "object" ? pd.profile_ext : {};
  return normalizeStateCode(ext.state || ext.us_state || pd.us_state || pd.state);
}
...
function surfaceHintsFromProfile(pd, contact, leadSourceTable) {
  const origin = pd && pd.originDetail && typeof pd.originDetail === "object" ? pd.originDetail : {};
  return {
    leadSourceTable: leadSourceTable || "",
    source: (contact && contact.source) || (pd && (pd.source || pd.lead_source)) || "",
    utmSource: origin.utm_source || (pd && pd.utm_source) || "",
    campaign: origin.utm_campaign || (pd && pd.utm_campaign) || "",
    whatsappId: (contact && (contact.whatsapp_id || contact.manychat_subscriber_id)) || "",
    manychatSubscriberId: (contact && contact.manychat_subscriber_id) || "",
  };
}

async function fetchPoliciesSoldMetrics(cfg, startIso, endExclusiveIso, dateFrom, dateTo, opts) {
  ...
  rows.forEach((row) => {
    if (String(row.lead_source_table || "") === "contacts" && row.lead_id) {
      contactIds.add(String(row.lead_id));
    }
  ...
  for (let i = 0; i < idList.length; i += 80) {
    ...
    try {
      const contacts = await restSelect(
        cfg,
        "contacts",
        `select=id,us_state,first_name,last_name,source,whatsapp_id,manychat_subscriber_id&id=in.(${inList})`
      );
      (contacts || []).forEach((c) => {
        if (c && c.id) contactMap.set(String(c.id), c);
      });
    } catch (e) {
      console.error("[crm-stage-transitions] sales contacts", e.message || e);
    }
    try {
      const states = await restSelect(
        cfg,
        "lead_state",
        `select=contact_id,us_state&contact_id=in.(${inList})`
      );
      (states || []).forEach((s) => {
        if (s && s.contact_id) leadStateMap.set(String(s.contact_id), s);
      });
    } catch (e) {
      console.error("[crm-stage-transitions] sales lead_state", e.message || e);
    }
  }
  ...
  const sales = rows.map((row) => {
    const pd = profileMap.get(String(row.lead_id) + "|" + String(row.lead_source_table)) || {};
    const contactId =
      String(row.lead_source_table || "") === "contacts"
        ? String(row.lead_id)
        : String(pd.contacts_contact_id || pd.contact_id || "").trim();
    const contact = contactId ? contactMap.get(contactId) || {} : {};
    const ls = contactId ? leadStateMap.get(contactId) || {} : {};
    const nameFromContact = [contact.first_name, contact.last_name].filter(Boolean).join(" ").trim();
    const surface = crmFunnelSurfaceFromHints(
      surfaceHintsFromProfile(pd, contact, row.lead_source_table)
    );
    return {
      leadId: row.lead_id,
      leadSourceTable: row.lead_source_table,
      name: displayNameFromProfile(pd) || nameFromContact || "Client",
      state:
        stateFromProfile(pd)
        || normalizeStateCode(contact.us_state)
        || normalizeStateCode(ls.us_state)
        || "",
      surface,
  ...
  const stateCode = String((opts && opts.stateCode) || "").trim().toUpperCase();
  const filtered = stateCode
    ? sales.filter((row) => (row.state || "UNKNOWN") === stateCode)
    : sales;
  ...
  filtered.forEach((row) => {
    const code = row.state || "UNKNOWN";
    byStateMap.set(code, (byStateMap.get(code) || 0) + 1);
  });
  const byState = LICENSED_STATES.map((state) => ({
    state,
    count: byStateMap.get(state) || 0,
  }));
  if ((byStateMap.get("UNKNOWN") || 0) > 0) {
    byState.push({ state: "UNKNOWN", count: byStateMap.get("UNKNOWN") });
  }
  ...
  return {
    ...
    sales: filtered.slice(0, 10),
    byState,
    daily: buildDailySoldSeries(
      filtered.map((row) => ({ changed_at: row.changedAt })),
      dateFrom,
      dateTo
    ),
  };
