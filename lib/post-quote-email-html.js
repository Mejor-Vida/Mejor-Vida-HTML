...
function buildEmailEN(name, quoteLow, quoteHigh, callScheduled, callDatetime) {
  ...
  const apptBlock = callScheduled
    ...
    : `<p>When you're ready to take the next step, schedule a call with Mejor Vida Insurance so we can review your health information, confirm the official quote, and lock in the price — no pressure, just a friendly conversation.</p>
       <div class="cta">${btn("Schedule a call with Mejor Vida Insurance", SCHEDULE_URL)}</div>`;
  ...
  return {
    ...
    html: wrapResendEmailHtml(
      `
<p>Hi ${name},</p>
<p>Great news — here's your approximate estimated quote range for final expense coverage:</p>
${quoteBlock}
...
<p>Based on your age, I wasn't able to generate an automated quote through our standard final expense carriers — the products I work with most commonly issue new coverage through age 89.</p>
...
<p>Según tu edad, no pude generar una cotización automática con nuestras compañías habituales de gastos finales — los productos con los que trabajo normalmente emiten cobertura nueva hasta los 89 años.</p>
...
function buildEmailES(name, quoteLow, quoteHigh, callScheduled, callDatetime) {
  ...
  const apptBlock = callScheduled
    ...
    : `<p>Cuando esté listo/a para el siguiente paso, agenda una llamada con Mejor Vida Seguros para tomar en cuenta sus datos de salud, confirmar la cotización oficial y dejar el precio fijo — sin presión.</p>
       <div class="cta">${btn("Agendar una llamada con Mejor Vida Seguros", SCHEDULE_URL)}</div>`;
  ...
  return {
    ...
    html: wrapResendEmailHtml(
      `
<p>Hola ${name},</p>
<p>¡Buenas noticias! Aquí está su estimado aproximado para cobertura de gastos finales:</p>
${quoteBlock}
