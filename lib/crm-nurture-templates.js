...
const { waQuoteUrl } = require("./whatsapp-cta");
...
const WHATSAPP_URL_ES = waQuoteUrl("es");
const WHATSAPP_URL_EN = waQuoteUrl("en");

function contactLang(contact) {
  const lang = String((contact && (contact.idioma || contact.language)) || "").toLowerCase();
...
function contactName(contact, lang) {
  const name =
    ((contact && contact.first_name) || ((contact && contact.full_name) || "").split(" ")[0] || "").trim() || "";
  if (name) return name;
...
function contactQuote(contact) {
  const p =
    contact && contact.payload && typeof contact.payload === "object" ? contact.payload : {};
  const low = sanitizeMoneyField(
    (contact && (contact.quote_low || contact.quoteLow)) || p.quoteLow || p.quote_low
  );
  const high = sanitizeMoneyField(
    (contact && (contact.quote_high || contact.quoteHigh)) || p.quoteHigh || p.quote_high
  );
  const anchor = sanitizeMoneyField(
    (contact && (contact.quote_anchor || contact.quoteAnchor)) || p.quoteAnchor || p.quote_anchor
  );
  const covRaw = (contact && (contact.coverage || contact.coverageAmount)) || p.coverageAmount;
  const cov = parseInt(String(covRaw ?? ""), 10);
  return {
    low,
    high,
    anchor,
    coverage: Number.isFinite(cov) && cov >= 2000 ? cov : 10000,
  };
}

function contactHasQuoteRate(contact) {
  const q = contactQuote(contact);
  return Boolean(q.low || q.high || q.anchor);
}

function formatCoverageUsd(n) {
  return "$" + Number(n).toLocaleString("en-US");
}

function welcomeQuoteBlock(contact, isEn) {
  const q = contactQuote(contact);
  if (!contactHasQuoteRate(contact)) return "";
  const range =
    q.low && q.high && q.low !== q.high ? `$${q.low} – $${q.high}` : `$${q.low || q.high || q.anchor}`;
  const per = isEn ? "/mo" : "/mes";
  const label = isEn
    ? `Approximate monthly estimate for ${formatCoverageUsd(q.coverage)} of coverage`
    : `Estimado mensual aproximado para ${formatCoverageUsd(q.coverage)} de cobertura`;
  return `<div class="quote-box">
        <div class="range">${range}${per}</div>
...
function welcomeCtaRow(isEn) {
  const scheduleBtn = isEn
    ? btn("Schedule a call with Mejor Vida Insurance", SCHEDULE_URL, "#fec963", "#111")
    : btn("Agendar una llamada con Mejor Vida Seguros", SCHEDULE_URL, "#fec963", "#111");
  const waBtn = isEn
    ? btn("WhatsApp", WHATSAPP_URL_EN, "#25D366", "#fff")
    : btn("WhatsApp", WHATSAPP_URL_ES, "#25D366", "#fff");
  return `<div style="text-align:center;padding:8px 0 24px;">${scheduleBtn}${waBtn}</div>`;
}
...
${welcomeQuoteBlock(contact, true)}
${
  contactHasQuoteRate(contact)
    ? `<p>This is an <strong>approximate estimate</strong>, not your official quote. To get the official quote and lock in the price, schedule a call with Mejor Vida Insurance. On that call we review your health information so the official quote price can be confirmed and locked in.</p>`
    : `<p>To get your official quote and lock in the price, schedule a call with Mejor Vida Insurance. On that call we review your health information so the official quote price can be confirmed and locked in.</p>`
}
${welcomeCtaRow(true)}
<p>Over the next few days I'll personally follow up to answer your questions — no pressure, just honest guidance.</p>
...
${welcomeQuoteBlock(contact, false)}
${
  contactHasQuoteRate(contact)
    ? `<p>Este es un <strong>estimado aproximado</strong>, no la cotización oficial. Para obtener su cotización oficial y dejar el precio asegurado, agenda una llamada con Mejor Vida Seguros. En esa llamada se toman en cuenta sus datos de salud para confirmar el precio oficial y dejarlo fijo.</p>`
    : `<p>Para obtener su cotización oficial y dejar el precio asegurado, agenda una llamada con Mejor Vida Seguros. En esa llamada se toman en cuenta sus datos de salud para confirmar el precio oficial y dejarlo fijo.</p>`
}
${welcomeCtaRow(false)}
<p>En los próximos días le daré seguimiento personalmente para responder sus preguntas — sin presión, sin compromiso, solo orientación honesta.</p>
${sig}`,
            logoUrl
          ),
        },
