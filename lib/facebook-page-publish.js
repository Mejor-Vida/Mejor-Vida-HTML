...
async function graphGet(path, params) {
  ...
  const u = new URL(`${GRAPH}/${String(path || "").replace(/^\//, "")}`);
...
async function fetchComment(commentId) {
  return graphGet(commentId, {
    fields: "id,message,from,post,parent,created_time",
  });
}
...
/** Update the message text on an existing Page post (does not change the photo). */
async function updatePostMessage(postId, message) {
  const { token } = pageCreds();
  if (!String(postId || "").trim()) throw new Error("Facebook update needs post_id");
  if (!String(message || "").trim()) throw new Error("Facebook update message is empty");
  return graphForm(`${GRAPH}/${postId}`, {
    message,
    access_token: token,
  });
}
