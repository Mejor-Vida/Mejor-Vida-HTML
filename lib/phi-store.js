...
let loggedPhiFallback = false;

function getPhiKey() {
  const primary = String(process.env.PHI_ENCRYPTION_KEY || "").trim();
  if (primary) return createHash("sha256").update(primary).digest();

  // Keep PHI write/read operational if dedicated key is missing.
  // This should be treated as a temporary safety fallback.
  const fallback = String(process.env.SUPABASE_SERVICE_ROLE_KEY || "").trim();
  if (fallback) {
    if (!loggedPhiFallback) {
      loggedPhiFallback = true;
      console.warn("PHI_ENCRYPTION_KEY missing; using SUPABASE_SERVICE_ROLE_KEY as PHI key fallback.");
    }
    return createHash("sha256").update(fallback).digest();
  }

  throw new Error("PHI_ENCRYPTION_KEY missing and no fallback key available");
}
