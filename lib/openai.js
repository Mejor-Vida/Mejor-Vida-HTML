...
Write your full answer in Spanish only.`
  ...
  console.log("[LLM_INPUT] context first 600 chars:", (messages.find((m) => m.role === "system")?.content || "").substring(0, 600));
  console.log("[LLM_INPUT] number of messages:", messages.length);
  console.log("[LLM_INPUT] last user message:", (messages[messages.length - 1]?.content || "").substring(0, 150));
