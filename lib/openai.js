...
- The user message begins with OUTPUT LANGUAGE. Write your entire answer in that language only. The context may be English, Spanish, or mixed — use it for facts only; do not copy its language if it differs from OUTPUT LANGUAGE.
...
async function getRAGAnswer(apiKey, userQuestion, contextChunks, languageHint, opts) {
  ...
  const outputSpanish = lh.startsWith("es") || lh === "spanish";
  const langBlock = outputSpanish
    ? "OUTPUT LANGUAGE: Spanish. Reply only in Spanish."
    : "OUTPUT LANGUAGE: English. Reply only in English.";
