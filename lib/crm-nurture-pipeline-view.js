...
const { contactedEmailTemplateForAttempt } = require("./crm-nurture-defaults");

const EMAIL_LABELS = {
  welcome: "Welcome",
  educational_day2: "Day 2 educational",
  contacted_educational: "Personal check-in",
  contacted_educational_1: "Personal check-in",
  contacted_educational_2: "What is final expense?",
  contacted_educational_3: "Julie's story",
  contacted_educational_4: "Don't wait too long",
};
...
function applyContactedEmailTemplates(timeline, settings, enrollmentStage) {
  if (String(enrollmentStage || "").toLowerCase() !== "contacted") return timeline || [];
  const rows = timeline || [];
  const contactedRecurring = rows.filter(
    (t) => t.task_type === "email" && t.payload && t.payload.recurring
  );
  return rows.map((t) => {
    if (t.task_type !== "email" || !(t.payload && t.payload.recurring)) return t;
    const idx = contactedRecurring.indexOf(t);
    const attempt = idx >= 0 ? idx + 1 : Number(t.payload.attempt) || 1;
    const template = contactedEmailTemplateForAttempt(settings, attempt);
    return Object.assign({}, t, {
      payload: Object.assign({}, t.payload, { template, attempt }),
    });
  });
}
...
function buildClientPipelineView(opts) {
  ...
  if (enrollment && (enrolled || includeStopped)) {
    ...
    if (String(enrollment.stage || "").toLowerCase() === "contacted") {
      ...
      timeline = applyContactedEmailTemplates(timeline, settings, enrollment.stage);
    }
...
/** Current step # in the nurture pipeline (matches Pipeline tab # column). */
function resolveNurtureStepSummary(opts) {
  const view = buildClientPipelineView(
    Object.assign({}, opts || {}, { includeStopped: false, contact: (opts && opts.contact) || null })
  );
  if (!view.enrolled) {
    return {
      nurture_step_number: null,
      nurture_step_total: null,
      nurture_step_label: null,
    };
  }
  const steps = view.steps || [];
  const total = steps.length;
  const next = steps.find((s) => s.is_next);
  if (next) {
    return {
      nurture_step_number: next.stageNumber,
      nurture_step_total: total || null,
      nurture_step_label: next.name || null,
    };
  }
  return {
    nurture_step_number: total || null,
    nurture_step_total: total || null,
    nurture_step_label: null,
  };
}
