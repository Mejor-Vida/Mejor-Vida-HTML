 ...
 * Views = traffic source × landing surface (V2 / V3 / Website).
 */
...
function buildFunnelViews() {
  ...
  ["facebook", "google", "direct"].forEach((source) => {
    ["v2", "v3", "website"].forEach((landingPage) => {
      ...
      views[id] = {
        id,
        label: SOURCE_LABELS[source] + " · " + LANDING_LABELS[landingPage],
        sources: [source],
        landingPage,
        entryLabel: landingPage === "website" ? "Website Home / Tools" : "Landing Page",
      };
    });
  });
...
function normalizeViewId(viewId) {
  const id = String(viewId || "facebook_v2").trim();
...
function getFunnelView(viewId) {
  const id = normalizeViewId(viewId);
  return FUNNEL_VIEWS[id] || FUNNEL_VIEWS.facebook_v2;
}
...
function composeViewId(source, landingPage) {
  return String(source || "facebook").trim() + "_" + String(landingPage || "v2").trim();
}

function parseViewId(viewId) {
  const id = normalizeViewId(viewId);
  const view = getFunnelView(id);
  if (!view) return { source: "facebook", landingPage: "v2", viewId: "facebook_v2" };
  const landingPage = view.landingPage || "v2";
