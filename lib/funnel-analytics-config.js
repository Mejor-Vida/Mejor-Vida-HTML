...
/** Licenses popup on V3 — side path, not a required Get Quote step. Events use tool=quote. */
const LICENSES_BRANCH = {
  id: "licenses",
  label: "Licenses",
  terminal: true,
  steps: [
    {
      ...
      match: { event_type: "click", step_name: "licenses_open", tool: "quote" },
    },
    {
      id: "license_state",
      label: "State license click",
      match: { event_type: "click", step_name: "license_state", tool: "quote" },
    },
  ],
};
...
const CONTACT_FIRST_BRANCH_ORDER = ["quote", "licenses", "calculator", "schedule", "whatsapp", "phone"];
...
function getBranchesForView(viewId) {
  ...
  if (view.landingPage === "website") {
    ...
    delete branches.bio;
    branches.licenses = LICENSES_BRANCH;
