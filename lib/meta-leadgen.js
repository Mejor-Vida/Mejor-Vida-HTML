...
function resolveLeadgenPageId(entry, value) {
  const entryId = entry && entry.id != null ? String(entry.id).trim() : "";
  const valuePageId = value && value.page_id != null ? String(value.page_id).trim() : "";
  if (entryId && entryId !== "0") return entryId;
  if (valuePageId && valuePageId !== "0") return valuePageId;
  return "";
}
...
function parseWebhookEntries(body) {
  ...
  for (const entry of body.entry) {
    ...
    for (const ch of changes) {
      ...
      const pageId = resolveLeadgenPageId(entry, v);
      leads.push({
        leadgenId,
        pageId,
...
async function processLeadgenEvent(meta, options = {}) {
  ...
  const pageId = String(meta.pageId || "").trim();
  if (expectedPageId && pageId && pageId !== "0" && pageId !== expectedPageId) {
    throw new Error(`Page id mismatch (got ${pageId}, expected ${expectedPageId})`);
  }
