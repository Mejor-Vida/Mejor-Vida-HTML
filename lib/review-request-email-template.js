...
function buildReviewRequestSubject({ language, firstName }) {
  ...
  if (useEs) {
    return n ? `¿Podría dejarnos una reseña, ${n}?` : "¿Podría dejarnos una reseña?";
  }
...
function buildReviewRequestPlainText({ language, firstName, reviewLink }) {
  ...
  if (useEs) {
    return (
      `${salutation}\n\n` +
      `Gracias por confiar en Mejor Vida Insurance. Su opinión ayuda a otras familias a encontrar ` +
      `orientación bilingüe sobre seguros de gastos finales aquí en Nebraska.\n\n` +
      `Si tuvo una buena experiencia con Julie, ¿podría dejarnos una reseña breve en Facebook? ` +
