...
const PRODUCTION_DRIVE_REDIRECT_URI =
...
async function getDriveAccessToken() {
  if (!hasDriveBackupCredentials()) {
    const err = new Error(
      "Google Drive backup is not connected. Open /api/staff/drive-auth once as admin@mejorvidainsurance.com."
    );
    err.code = "DRIVE_NOT_CONFIGURED";
    throw err;
  }
  const { refreshToken } = driveOAuthConfig();
  const client = driveOAuthClient(PRODUCTION_DRIVE_REDIRECT_URI);
