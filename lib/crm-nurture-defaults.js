...
const DEFAULT_CRM_NURTURE_SETTINGS = {
  ...
  retained_days: 365,
  loyal_days: 730,
  /** Automated Telnyx SMS multi-state curfew / holiday guardrails */
  compliance: {
...
const AUTO_ENROLL_NEW_STAGES = new Set(["new", "new_lead", "new_contact", "initiated"]);
const AUTO_ENROLL_CONTACTED_STAGES = new Set([
  "contacted",
  "attempting_contact",
  "call_scheduled",
]);
...
/** New + Contacted only — do not auto-enroll clients, sold, retained, or lost. */
function resolveAutoEnrollStage(pipelineStage) {
  const s = String(pipelineStage || "new").trim().toLowerCase();
  if (AUTO_ENROLL_NEW_STAGES.has(s)) return "new";
  if (AUTO_ENROLL_CONTACTED_STAGES.has(s)) return "contacted";
  return null;
}
