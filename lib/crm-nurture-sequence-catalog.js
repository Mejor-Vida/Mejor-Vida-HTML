...
const { DEFAULT_CRM_NURTURE_SETTINGS, contactedEmailTemplates, resolveDailySummaryRecipients } = require("./crm-nurture-defaults");
...
const { getWeeklyBlogDigestEmailPreview } = require("./crm-weekly-blog-digest-email");
...
function buildContactedSteps(settings, steps) {
  ...
  const templates = contactedEmailTemplates(settings);
  const phaseLabel = "Stage: Contacted — Long-term nurture (educational email series)";

  templates.forEach((tplKey, idx) => {
    const topicNum = idx + 1;
    pushStep(steps, {
      phase: "contacted",
      phase_label: phaseLabel,
      day: emailDays * topicNum,
      when:
        topicNum === 1
          ? `${emailDays} days after Contacted — then every ${emailDays} days (topic ${topicNum} of ${templates.length})`
          : `+${emailDays} days — topic ${topicNum} of ${templates.length}`,
      channel: "email",
      channel_label: channelLabel("email"),
      template: tplKey,
      audience: "Lead",
      preview: previewEmail(tplKey, settings),
      recurring: topicNum === 1,
    });
  });
}
  ...
  pushStep(steps, {
    ...
    preview: {
      ...
      html: [
        ...
        "<h3>Call or text yourself (2)</h3>",
        "<ul>",
        "<li>Lead without email — New · no email · no SMS opt-in</li>",
        "</ul>",
        "<h3>CRM scheduler notes due today (1)</h3>",
        "<ul>",
        "<li>Call about application — due 2:00 PM</li>",
        "</ul>",
        "<h3>Timed nurture calls (1)</h3>",
        "<ul>",
    ...
    note:
      `Internal morning digest sent to ${recipientLabel} — not sent to leads. Lists New/Contacted people Julie must call or text herself (no email and/or no SMS opt-in), CRM scheduler notes due today, timed nurture call tasks, and a count of client emails/SMS scheduled for the day. After Engaged, only the scheduler note appears on the date Julie picked.`,
  });
}
...
function buildNewsletterStep(settings, steps) {
  ...
  const preview = getWeeklyBlogDigestEmailPreview();

  pushStep(steps, {
    phase: "weekly_fb_post",
    phase_label: "Weekly blog digest — email (all stages except unsubscribed)",
    day: null,
    ...
    preview,
    note:
      "Example shown: condensed weekly blog digest (short story teasers + article links). " +
      "Review/import via /staff/weekly-email-review.html before the Sunday send.",
    recurring: true,
  });
}
