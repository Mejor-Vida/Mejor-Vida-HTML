/**
 * Sunday weekly newsletter: research → compose → save issue → email julie@ and admin@.
 */
...
function writeLocalPreview(digest, parts, partsEn) {
  try {
...
async function runWeeklyNewsletter(opts) {
  ...
  if (toAdmin) {
    ...
    picked = pickThree(harvested.candidates, sources);
  ...
  const previewFile = writeLocalPreview(digest, parts, partsEn);
