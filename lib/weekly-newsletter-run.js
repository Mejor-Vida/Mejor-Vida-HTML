...
async function runWeeklyNewsletter(opts) {
  ...
  const fromBrief = !!opts.fromBrief;
  ...
  if (toAdmin) {
    ...
    researchBrief = loadResearchBrief(window.weekKey);
    if (!researchBrief || !Array.isArray(researchBrief.selected) || researchBrief.selected.length !== 3) {
      throw new Error("No saved research brief with 3 selected stories. Run Stage 1 first or write tools/weekly-newsletter/out/research-brief-*.json");
    }
  ...
  const staff = await sendStaffWeeklyNewsletter(issue, {
    englishParts: partsEn,
    staffNoteHtml: fromBrief
      ? `<p style="font-size:13px;color:#666;margin:0 0 12px;padding:10px 12px;background:#f5f5f5;border-radius:6px">` +
        `<strong>Replacement letter</strong> — the earlier send used repeat evergreen explainers. ` +
        `This version uses last week’s dated news (Sep 6–12). Spanish to julie@; English to admin@. ` +
        `Client send is still Staff → Weekly emails → Send now.` +
        `</p>`
      : undefined,
  });
