...
function parseUsState(v) {
  const s = asValue(v);
  if (!s) return null;
  return normalizeUsStateAbbr(s) || null;
}
...
function coerceNormalized(rawByAlias) {
  ...
  const qs = asValue(rawByAlias.quote_status);
  if (qs) out.quote_status = qs.slice(0, 80);
  const qe = asValue(rawByAlias.quote_error);
  if (qe) out.quote_error = qe.slice(0, 500);
  ...
  const adId = String(rawByAlias.meta_ad_id || "").replace(/\D/g, "");
  if (adId.length >= 8 && adId.length <= 24) out.meta_ad_id = adId;
  return out;
}
