...
function isQuotePageOrigin(originDetail) {
  const path = gastosFinalesLandingPath(originDetail).split("?")[0];
  return /\/quote\.html$/i.test(path);
}

function shouldSendMetaCapiWebsite(originDetail, body) {
  if (isQuotePageOrigin(originDetail)) return true;
  return isSpanishGastosFinalesLanding(originDetail, body);
}

function shouldSendMetaCapiForLead(leadSource, originDetail) {
  if (leadSource === "facebook_landing_gastos_finales") return true;
  if (leadSource === "nebraska_quote_page") return true;
...
function capiFbcFromOrigin(metaFbc) {
  ...
  // Pass through browser _fbc exactly — do not rebuild or trim the fbclid segment.
  const match = fromClient.match(/^fb\.1\.(\d+)\.(.+)$/);
  if (!match) return null;
  const fbclidPart = match[2];
  if (!fbclidPart || /^(fbclid|\{\{fbclid\}\}|test|placeholder)$/i.test(fbclidPart)) {
    return null;
  }
  return fromClient;
}
...
function capiEventSourceUrl(originDetail) {
  ...
  return "https://www.mejorvidainsurance.com/gastos-finales-ads-v3/";
}
...
async function sendMetaCapiWebsiteEvent({
  ...
  try {
    ...
    } else if (!shouldSendMetaCapiWebsite(originDetail, body)) {
      return { skipped: true, reason: "surface not eligible for Meta CAPI" };
    }
