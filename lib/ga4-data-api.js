...
/**
 * GA4 linked Google Ads metrics often return empty rows for a one-day range
 * (especially "today"). Query the prior day too, then filter to the requested range.
 */
function resolveGa4AdsQueryRange(startDate, endDate) {
  const start = String(startDate || "").trim();
  const end = String(endDate || "").trim();
  if (start === end && /^\d{4}-\d{2}-\d{2}$/.test(start)) {
    return {
      queryStart: addDaysYmd(start, -1),
      queryEnd: end,
      filterStart: start,
      filterEnd: end,
    };
...
function dateInRangeYmd(date, start, end) {
...
async function fetchLinkedGoogleAdsTotals(startDate, endDate) {
  const { analyticsData, propertyId } = await getAnalyticsClient();
  const range = resolveGa4AdsQueryRange(startDate, endDate);
  ...
  const dimensionAttempts = [
    ...
    [{ name: "date" }, { name: "googleAdsAccountName" }],
  ];
  if (range.queryStart === range.filterStart && range.queryEnd === range.filterEnd) {
    dimensionAttempts.push([{ name: "sessionCampaignName" }], [{ name: "googleAdsAccountName" }]);
  }
  ...
  for (const dimensions of dimensionAttempts) {
    try {
      const data = await runReport(analyticsData, propertyId, {
        dateRanges: [{ startDate: range.queryStart, endDate: range.queryEnd }],
        dimensions,
        metrics: metricDefs,
        limit: 10000,
      });
      ...
      (data.rows || []).forEach((row) => {
        const dims = parseDimensionRow(row, data.dimensionHeaders);
        const rowDate = formatGa4Date(dims.date);
        if (rowDate && !dateInRangeYmd(rowDate, range.filterStart, range.filterEnd)) {
          return;
        }
        const metrics = parseMetricRow(row, data.metricHeaders);
...
/**
 * Daily Google Ads impressions/clicks from GA4 (aggregated by date).
 */
async function fetchLinkedGoogleAdsDaily(startDate, endDate) {
  const { analyticsData, propertyId } = await getAnalyticsClient();
  const range = resolveGa4AdsQueryRange(startDate, endDate);
  ...
  for (const dimensions of dimensionAttempts) {
    try {
      const data = await runReport(analyticsData, propertyId, {
        dateRanges: [{ startDate: range.queryStart, endDate: range.queryEnd }],
        dimensions,
        metrics: [{ name: "advertiserAdImpressions" }, { name: "advertiserAdClicks" }],
        limit: 10000,
        orderBys: [{ dimension: { dimensionName: "date" } }],
      });
      ...
      (data.rows || []).forEach((row) => {
        const dims = parseDimensionRow(row, data.dimensionHeaders);
        const metrics = parseMetricRow(row, data.metricHeaders);
        const date = formatGa4Date(dims.date);
        if (!date || !dateInRangeYmd(date, range.filterStart, range.filterEnd)) return;
        const prev = byDate.get(date) || { date, impressions: 0, clicks: 0 };
        prev.impressions += metrics.advertiserAdImpressions || 0;
        prev.clicks += metrics.advertiserAdClicks || 0;
  ...
  throw lastError || new Error("Could not fetch daily Google Ads metrics from GA4");
}
