...
async function issueToken(cfg, { leadId, leadSourceTable, recipientEmail, issuedBy }) {
  const raw = generateRawToken();
  const tokenHash = hashToken(raw);
  const expiresAt = new Date(Date.now() + TOKEN_TTL_MS).toISOString();
  const row = await restInsert(cfg, "medical_intake_access_tokens", {
...
async function validateToken(cfg, rawToken) {
  ...
  const rows = await restSelect(
    ...
    `select=id,lead_id,lead_source_table,expires_at,used_at,status&token_hash=eq.${encodeURIComponent(tokenHash)}&limit=1`
  );
