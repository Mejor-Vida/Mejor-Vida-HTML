 ...
 */
const { loadSettings } = require("./crm-nurture-engine");
const {
  wrapNewsletterHtml,
  leadEmailCtaRow,
  contactName,
...
async function listWeeklyEmailRecipients(supabaseUrl, serviceKey, settings) {
  ...
  for (const row of profiles || []) {
    ...
    eligible.push({
      ...
      contacts_contact_id: pd.contacts_contact_id || pd.contact_id || null,
...
async function sendWeeklyNewsletterIssue(supabaseUrl, serviceKey, issue, opts) {
  ...
  return {
    ...
    sent,
    failed,
    skipped: counts.skipped_no_email + counts.skipped_stage + counts.skipped_rollout,
    ...counts,
    errors,
    sent_at: now,
  };
