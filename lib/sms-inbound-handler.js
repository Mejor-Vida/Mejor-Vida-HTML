...
function parseTelnyxInbound(body) {
  ...
  const fromPhone = (payload.from && (payload.from.phone_number || payload.from.number)) || "";
  const msgBody = String(payload.text || "").trim();
  if (!fromPhone) return null;
  return { fromPhone: fromPhone.trim(), msgBody };
}
...
function validateTelnyxWebhookSecret(req) {
  ...
  if (q && String(q).trim() === expected) return true;
