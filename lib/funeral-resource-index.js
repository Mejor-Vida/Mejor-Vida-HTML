...
function loadGuideCities() {
  ...
  return [omaha, lincoln, grandIsland, ...kansasCities, ...coloradoCities];
}
...
function applyGplOverlay(home, cityName) {
  let overlays = [];
  try {
    overlays = require("../scripts/funeral-directory/nebraska-gpls");
  } catch {
    overlays = [];
  }
...
function gplKind(source, notes) {
  ...
  if (
    /no publican gpl|do not post a gpl|promedio de kansas|kansas average, not|promedio de colorado|colorado average, not/.test(
      t
    )
  )
    return "stateAverage";
...
function harvestCity(city) {
  ...
  return {
    ...
    stateNameEs: city.stateNameEs,
    stateNameEn: city.stateNameEn,
    nameEs: city.nameEs,
    nameEn: city.nameEn,
...
function listingPaths(stateSlug, slug) {
  return {
    pathEs: `/funerarias-cementerios/${stateSlug}/${slug}.html`,
    pathEn: `/en/funeral-homes-cemeteries/${stateSlug}/${slug}.html`,
  };
}
...
function collectStates(places) {
  ...
  places.forEach((p) => {
    ...
    states.push({
      ...
      pathEs: `/funerarias-cementerios/${p.stateSlug}.html`,
      pathEn: `/en/funeral-homes-cemeteries/${p.stateSlug}.html`,
    });
  });
  return states;
}
