...
/** Match Meta ad set names to CRM Facebook destination filters. */
function metaAdSetMatchesVariant(adSetName, variant) {
  if (!variant) return true;
  const surface = metaAdSetSurface(adSetName);
  if (variant === "landing") return surface === "landing" || surface === "v3";
  if (variant === "v3") return surface === "landing" || surface === "v3";
      ...
      async function queryAdSets(includeActions) {
        const params = new URLSearchParams({
          fields: includeActions
            ? "adset_name,impressions,clicks,spend,actions"
