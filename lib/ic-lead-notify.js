...
/**
 * True if this email or phone already exists on a quoter submission (skip duplicate IC email).
 */
...
async function sendAppointmentLeadNotification(lead, { supabaseUrl, serviceKey }) {
  try {
    ...
    if (supabaseUrl && serviceKey) {
      const duplicate = await quoterLeadAlreadyExists(
        supabaseUrl,
        serviceKey,
        lead.email,
        lead.phone
      ...
      if (duplicate) {
        console.log(
          "[NOTIFY] skipped — quoter lead already exists for",
          lead.email || lead.phone || "contact"
        );
        return { skipped: true, reason: "quoter_lead_exists" };
      }
    }
