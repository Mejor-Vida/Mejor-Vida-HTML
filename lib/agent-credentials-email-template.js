 ...
 * vCard link + Nebraska license verification + Julie headshot.
 */
...
const LICENSE_LOOKUP_URL = String(
  process.env.NEBRASKA_LICENSE_VERIFY_URL || "https://sbs.naic.org/solar-external-lookup/"
).trim();
...
function buildAgentCredentialsSubject({ language, firstName }) {
  ...
  if (useEs) {
    return n
      ? `${n}, contacto y credenciales de Julie — agente licenciada en Nebraska`
      : "Contacto y credenciales de Julie — agente licenciada en Nebraska";
  }
  return n
    ? `${n}, Julie's contact & license info — Nebraska licensed agent`
    : "Julie's contact & license info — Nebraska licensed agent";
}
...
function buildAgentCredentialsIntroPlainText({ language, firstName }) {
  ...
  if (useEs) {
    return (
      ...
      `oficial de contacto y licencia como agente licenciada en seguros de vida y salud en Nebraska.`
    );
  }
  ...
  return (
    ...
    `licensing information. I am a licensed life and health insurance agent in Nebraska.`
  );
}
...
function buildAgentCredentialsPlainDetails({ language }) {
  ...
  if (useEs) {
    return (
      ...
      `Licencia de productor #${LICENSE_NUMBER}\n\n` +
      `Tarjeta de contacto (adjunta): ${VCF_URL}\n` +
      `Verificar licencia: ${LICENSE_LOOKUP_URL}\n` +
  ...
  return (
    `Julie Braunsroth · Mejor Vida Insurance LLC\n` +
    `Nebraska Producer License #${LICENSE_NUMBER}\n\n` +
    `Contact card (attached): ${VCF_URL}\n` +
    `Verify license: ${LICENSE_LOOKUP_URL}\n` +
...
function buildAgentCredentialsHtmlBlock({ language }) {
  ...
  const verifyLabel = useEs ? "Consultar licencia (NAIC)" : "Look up license (NAIC)";
  ...
  return (
    ...
    `<span style="font-size:13px;color:#64748b;">English · Español · Nebraska</span>` +
    `</td></tr></table>` +
    licensePhotoHtml +
    ...
    `<a href="${LICENSE_LOOKUP_URL.replace(/"/g, "&quot;")}" style="display:inline-block;padding:14px 22px;background:#1a4d8c;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:600;font-size:15px;margin:6px 4px;">${verifyLabel}</a>` +
    `</p>` +
    ...
    (useEs
      ? `📞 <a href="tel:+14024405438" style="color:#3b82f6;">402-440-5438</a> · ` +
        ...
        `<a href="${ABOUT_JULIE_URL}" style="color:#3b82f6;">Sobre Julie</a>`
      : `📞 <a href="tel:+14024405438" style="color:#3b82f6;">402-440-5438</a> · ` +
        `💬 <a href="https://wa.me/14024405438" style="color:#3b82f6;">WhatsApp</a> · ` +
        `<a href="${ABOUT_JULIE_URL}" style="color:#3b82f6;">About Julie</a>`) +
    `</p></div></div></div>`
  );
}
