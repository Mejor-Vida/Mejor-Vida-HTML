...
async function upsertHsContact(token, phone, {
  firstName, lastName, email, language, usState, age, gender, isSmoker, quoteLow, quoteHigh,
}) {
  ...
  const properties = {
    ...
    ...(quoteLow && { mvi_quote_low: String(quoteLow).slice(0, 200) }),
    ...(quoteHigh && { mvi_quote_high: String(quoteHigh).slice(0, 200) }),
...
async function syncContactToHubspot(supabaseUrl, supabaseKey, hubspotToken, pipelineId, contactId) {
  ...
  const hsContactId = await upsertHsContact(hubspotToken, phone, {
    ...
    quoteLow: leadState && leadState.quote_low,
    quoteHigh: leadState && leadState.quote_high,
  });
