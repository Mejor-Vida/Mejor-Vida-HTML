...
const META_USER_OAUTH_SCOPES = ["ads_read", "read_insights"].join(",");
const META_SYSTEM_USER_OAUTH_SCOPES = ["business_management", "ads_read", "read_insights"].join(",");
...
function buildMetaOAuthUrl(redirectUri, options) {
  const { appId } = getMetaAppConfig();
  const userOnly = !!(options && options.userOnly);
  const scope = userOnly ? META_USER_OAUTH_SCOPES : META_SYSTEM_USER_OAUTH_SCOPES;
  ...
  if (userOnly) params.set("state", "user");
...
module.exports = {
  ...
  META_USER_OAUTH_SCOPES,
  META_SYSTEM_USER_OAUTH_SCOPES,
  getMetaAppConfig,
