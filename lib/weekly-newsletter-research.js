...
function sourceNameAllowed(sourceName, names) {
  const hay = String(sourceName || "").toLowerCase();
  if (!hay) return false;
  return (names || []).some((n) => {
    const needle = String(n || "").toLowerCase().trim();
    if (!needle) return false;
    if (hay === needle) return true;
    // Short names like "AP" must be a token so they do not match "Telegraph".
    if (needle.length < 4) {
      return hay.split(/[^a-z0-9]+/).filter(Boolean).includes(needle);
    }
...
function isAllowedPublisher(item, sources) {
  ...
  return sourceNameAllowed(item.sourceName, sources.allow_source_names || []);
}

/** Prefer the publisher page. Google News article URLs often 401 in Stage 1 validation. */
function candidatePageUrl(item) {
  const src = String(item.sourceUrl || "").trim();
  const link = String(item.link || "").trim();
  const srcHost = hostnameOf(src);
  if (/^https?:\/\//i.test(src) && srcHost && srcHost !== "news.google.com") return src;
  return link;
}
...
function toCandidate(item, window, sources, via) {
  ...
  const pageUrl = candidatePageUrl(item);
  return {
    title: item.title,
    url: pageUrl,
    source_name: item.sourceName || hostnameOf(item.sourceUrl) || "News",
    source_url: item.sourceUrl || pageUrl,
