 ...
 */
/**
 * Newest harvest batch holding rows for a state. Superseded batches stay in the
 * table, and the engine takes the cheapest row it is given, so an old harvest
 * could otherwise win a quote with a price the carrier no longer offers. Scoped
 * per state because a later harvest may cover only some states.
 */
async function latestIntegrityBatchForState(base, serviceKey, state) {
  const params = new URLSearchParams({
    state: `eq.${state}`,
    underwriting_mode: "eq.fully_underwritten",
    select: "harvest_batch_id",
    order: "harvest_batch_id.desc",
    limit: "1",
  });
  ...
  return rows.length ? rows[0].harvest_batch_id : null;
}
...
async function fetchTermIntegrityPremiums(
  ...
  const stateCode = String(state || "NE").toUpperCase();
  const batch = await latestIntegrityBatchForState(base, serviceKey, stateCode);
  const all = [];
  ...
  for (let offset = 0; offset < 50000; offset += pageSize) {
    ...
    if (batch) params.set("harvest_batch_id", `eq.${batch}`);
    const r = await fetch(`${base}/rest/v1/term_integrity_premiums?${params}`, {
      headers: restHeaders(serviceKey),
    });
    const text = await r.text();
