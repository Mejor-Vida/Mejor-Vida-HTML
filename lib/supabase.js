...
async function findManychatLeadsByPhone(supabaseUrl, serviceKey, phone) {
  ...
  const urlWith = `${base}?phone=eq.${q}&staff_hidden_at=is.null&select=id,email,phone,pipeline_stage,drop_off`;
  const urlNo = `${base}?phone=eq.${q}&select=id,email,phone,pipeline_stage,drop_off`;
