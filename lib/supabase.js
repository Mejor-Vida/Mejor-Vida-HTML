...
async function findManychatLeadsByPhone(supabaseUrl, serviceKey, phone) {
  ...
  const url = `${supabaseUrl.replace(/\/$/, "")}/rest/v1/manychat_leads?phone=eq.${q}&select=id,email,phone,pipeline_stage,drop_off`;
  const r = await fetch(url, {
...
async function findManychatLeadBySubscriberId(supabaseUrl, serviceKey, subscriberId) {
  ...
  const url = `${supabaseUrl.replace(/\/$/, "")}/rest/v1/manychat_leads?manychat_subscriber_id=eq.${q}&select=id,phone,email,first_name,last_name&limit=1`;
