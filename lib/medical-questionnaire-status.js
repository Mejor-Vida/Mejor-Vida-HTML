...
/** Days after send that a lead stays on the dashboard "Medical pending" list. */
const MEDICAL_PENDING_WINDOW_DAYS = 14;
const MEDICAL_PENDING_WINDOW_MS = MEDICAL_PENDING_WINDOW_DAYS * 24 * 60 * 60 * 1000;
...
/** True if the questionnaire was sent within the pending window (default 14 days). */
function isWithinMedicalPendingWindow(token, nowMs) {
  if (!token || !token.created_at) return false;
  const sentAt = new Date(token.created_at).getTime();
  if (!Number.isFinite(sentAt)) return false;
  const now = Number.isFinite(nowMs) ? nowMs : Date.now();
  return now - sentAt <= MEDICAL_PENDING_WINDOW_MS;
}
 ...
 * Leads drop off after MEDICAL_PENDING_WINDOW_DAYS unless the form is resent.
 */
async function countMedicalPendingLeads(cfg) {
  ...
  latestUnusedTokenByKey.forEach((unusedToken, key) => {
    ...
    if (status === "awaiting") {
      if (isWithinMedicalPendingWindow(activeToken || unusedToken, nowMs)) count += 1;
      return;
    }
    if (!activeToken && isAwaitingFromUnusedToken(latestSub, unusedToken)) {
      if (isWithinMedicalPendingWindow(unusedToken, nowMs)) count += 1;
    }
  });
...
module.exports = {
  ...
  MEDICAL_PENDING_WINDOW_DAYS,
  isWithinMedicalPendingWindow,
};
