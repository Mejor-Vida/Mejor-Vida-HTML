...
function buildIndex() {
  ...
  for (const listing of listings()) {
    ...
    for (const n of names) {
      ...
      if (n === "nebraska" || n === "kansas") continue;
...
function homeMatchIsJustCity(hay, home, listing) {
  ...
  if (!city || city.length < 4 || !homeFold.includes(city)) return false;
  const rest = homeFold
    .replace(city, " ")
    .split(" ")
    .filter((t) => t.length >= 4 && !GENERIC_HOME_TOKENS.has(t));
  return !rest.some((tok) => hasPhrase(hay, tok));
}
...
function askCity(isSpanish) {
  const dir = isSpanish
    ...
    : `${SITE}/en/funeral-homes-cemeteries.html`;
  return isSpanish
    ? `Puedo leer las listas generales de precios que ya reunimos para funerarias de Nebraska y Kansas. ¿En qué ciudad está? Por ejemplo Lincoln, Omaha u Olathe.\n\nTambién puede abrir el directorio: ${dir}`
    : `I can read the general price lists we have already gathered for funeral homes in Nebraska and Kansas. Which city are you in? For example Lincoln, Omaha, or Olathe.\n\nYou can also open the directory: ${dir}`;
}
