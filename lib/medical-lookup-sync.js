...
async function searchProviders(opts) {
  ...
  if (term) {
    ...
    if (nameParts.first_name) params.set("first_name", nameParts.first_name);
    if (nameParts.last_name) params.set("last_name", nameParts.last_name);
    ...
    if (!results.length && nameParts.last_name && state) {
      ...
      fallback.set("last_name", nameParts.last_name);
  ...
  const orgDirectory = state
    ...
    : [];
  const items = await buildProviderItems(results, zip, geo, radius, orgDirectory);
