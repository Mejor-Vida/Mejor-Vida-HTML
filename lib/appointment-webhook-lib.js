...
async function processAppointmentWebhook(input, options = {}) {
  ...
  async function sendIcNotify() {
    if (skipIcNotify) return null;
    return sendAppointmentLeadNotification(
      {
        ...lead,
  ...
  if (callScheduledAt) {
    ...
    if (existingAt && Math.abs(Date.parse(existingAt) - Date.parse(callScheduledAt)) <= 60_000) {
      const notifyResult = await sendIcNotify();
      return {
        ok: true,
        deduped: true,
  ...
  if (!skipIcNotify) {
    notifyResult = await sendIcNotify();
  }

  try {
    const { autoEnrollCrmLead } = require("./crm-nurture-engine");
    await autoEnrollCrmLead(
      { supabaseUrl, serviceKey },
      {
        leadId: contactId,
        ...
        stage: "contacted",
        contactId,
        actor: "appointment_webhook",
      }
    );
  } catch (e) {
    console.error("[appointment-webhook] nurture enroll", e && e.message ? e.message : e);
  }
