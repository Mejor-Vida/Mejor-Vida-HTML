...
function parseDigestStories(html, weekKey, origin) {
  ...
  while ((m = re.exec(html))) {
    ...
    const digestAnchor = `${base}/blog/weekly-insurance-update-${weekKey}.html#story${slot}`;
    const articleHref = (body.match(/<a class="btn btn-primary"[^>]*href="([^"]+)"/i) || [])[1] || "";
    let story_url = digestAnchor;
    if (articleHref) {
      if (/^https?:\/\//i.test(articleHref)) {
        story_url = articleHref.split("#")[0];
      } else {
        const file = articleHref.replace(/^\.\//, "").split("/").pop();
        if (file && /\.html$/i.test(file) && !/^weekly-insurance-update-/i.test(file)) {
          story_url = `${base}/blog/${file}`;
        }
      }
    }
    stories.push({
