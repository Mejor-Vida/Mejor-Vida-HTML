...
async function enrichScheduledCalls(supabaseUrl, serviceKey, calls) {
  return Promise.all(
    (calls || []).map(async (task) => {
      ...
      const first = contact && contact.first_name ? String(contact.first_name).trim() : "";
      const last = contact && contact.last_name ? String(contact.last_name).trim() : "";
      const displayName = [first, last].filter(Boolean).join(" ").trim() || "Lead";
      return Object.assign({}, task, {
        display_name: displayName,
        phone: contact && contact.phone ? String(contact.phone).trim() : "",
      });
    })
  );
}
...
async function fetchScheduledCallTasksForToday(supabaseUrl, serviceKey, settings) {
  ...
  const [nurtureTasks, enrollments] = await Promise.all([
    sbFetch(
      supabaseUrl,
      serviceKey,
    ...
    sbFetch(
      supabaseUrl,
      serviceKey,
      `/crm_nurture_enrollments?status=eq.active&stage=eq.new&select=id,lead_id,lead_source_table,contact_id,stage&limit=500`
    ),
  ]);
...
async function buildDailySummaryData(cfg) {
  ...
  const scheduledCalls = await fetchScheduledCallTasksForToday(supabaseUrl, serviceKey, settings);
...
async function sendDailySummaryEmail(cfg) {
  ...
  (data.new_call_tasks || []).forEach((c) => {
    const when = new Date(c.due_at).toLocaleString("en-US", { timeZone: data.timezone });
    const name = c.display_name || "Lead";
    const phone = c.phone ? ` · ${c.phone}` : "";
    lines.push(`<li><strong>${name}</strong>${phone} — Attempt #${c.attempt_number} · due ${when}</li>`);
  });
