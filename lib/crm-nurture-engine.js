...
async function enrollLead(cfg, opts) {
  ...
  if (!STAGES_WITH_AUTO_NURTURE.has(stage)) {
  ...
  const tasks = stage === "new" ? buildNewTasks(enrollment, settings) : buildContactedTasks(enrollment, settings);
  await insertTasks(supabaseUrl, serviceKey, enrollment.id, tasks);

  return { ok: true, enrollment_id: enrollment.id, tasks: tasks.length };
}
