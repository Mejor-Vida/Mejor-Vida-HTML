...
const { normalizeUsStateAbbr, stateFromRecord, applyLocationFields } = require("./us-state-timezone");
    ...
    try {
      const ls = await sbFetch(
        ...
        `/lead_state?contact_id=eq.${encodeURIComponent(
          contact.id
        )}&select=quote_low,quote_high,us_state&limit=1`
      );
      if (ls && ls[0]) {
        ...
        if (!contact.us_state && ls[0].us_state) contact.us_state = ls[0].us_state;
      }
    } catch (_) {
...
async function fetchLeadContactRaw(supabaseUrl, serviceKey, leadId, leadSourceTable, contactId) {
  if (contactId) {
    const rows = await sbFetch(
      supabaseUrl,
      serviceKey,
        `/contacts?id=eq.${contactId}&select=id,first_name,last_name,phone,email,us_state,language,idioma,vcf_sent_at&limit=1`
    );
  ...
  if (cid) {
    const rows = await sbFetch(
      supabaseUrl,
      serviceKey,
      `/contacts?id=eq.${cid}&select=id,first_name,last_name,phone,email,us_state,language,idioma,vcf_sent_at&limit=1`
    );
    if (rows && rows[0]) return rows[0];
  }
  if (leadSourceTable === "contacts") {
    const rows = await sbFetch(
      supabaseUrl,
      serviceKey,
      `/contacts?id=eq.${leadId}&select=id,first_name,last_name,phone,email,us_state,language,idioma,vcf_sent_at&limit=1`
    );
    if (rows && rows[0]) return rows[0];
  }
...
async function resolveLeadHints(supabaseUrl, serviceKey, leadId, leadSourceTable, contactId) {
  ...
  return {
    ...
    us_state:
      stateFromRecord(contact) ||
      stateFromRecord(profile) ||
      "",
  };
}
...
async function resolveLeadStateCode(supabaseUrl, serviceKey, leadId, leadSourceTable, opts) {
  ...
  const pd = profiles && profiles[0] ? profiles[0].profile_data : {};
  const fromProfile =
    normalizeUsStateAbbr((pd && (pd.us_state || pd.state_code || pd.state || pd.quote_state)) || "") ||
    normalizeStateCode((pd && (pd.state_code || pd.state || pd.quote_state)) || "");
  if (fromProfile) return fromProfile;

  if (leadSourceTable === "contacts" && leadId) {
    try {
      const rows = await sbFetch(
        supabaseUrl,
        serviceKey,
        `/contacts?id=eq.${encodeURIComponent(leadId)}&select=us_state&limit=1`
      );
      const st = normalizeUsStateAbbr(rows && rows[0] && rows[0].us_state);
      if (st) return st;
    } catch (_) {}
  }
  ...
  if (leadSourceTable === "quote_lead_submissions") {
    ...
    if (rows && rows[0]) {
      ...
      const st =
        normalizeUsStateAbbr(rows[0].state_code || p.state || p.state_code || p.us_state) ||
        normalizeStateCode(rows[0].state_code || p.state || p.state_code || "");
      if (st) return st;
    }
  }
  ...
  if (leadSourceTable === "fex_email_quotes") {
    ...
    const st =
      normalizeUsStateAbbr(rows && rows[0] && rows[0].state_code) ||
      normalizeStateCode(rows && rows[0] && rows[0].state_code);
    if (st) return st;
  }

  return allowDefault ? "NE" : "";
}
...
async function resolveLeadIdentity(supabaseUrl, serviceKey, leadId, leadSourceTable, contactId) {
  ...
  if (!displayName && leadSourceTable === "quote_lead_submissions") {
    try {
      const rows = await sbFetch(
        ...
        `/quote_lead_submissions?id=eq.${encodeURIComponent(
          leadId
        )}&select=first_name,last_name,phone,email,state_code,payload&limit=1`
      );
      const row = rows && rows[0] ? rows[0] : null;
      if (row) {
        ...
        const p = row.payload && typeof row.payload === "object" ? row.payload : {};
        if (!hints.us_state) {
          hints.us_state = normalizeUsStateAbbr(row.state_code || p.state || p.state_code || p.us_state);
        }
      }
    } catch (_) {}
  }
  let usState = normalizeUsStateAbbr(hints.us_state);
  if (!usState) {
    usState = await resolveLeadStateCode(supabaseUrl, serviceKey, leadId, leadSourceTable, {
      allowDefault: false,
    });
  }
  const identity = {
  ...
  applyLocationFields(identity, usState, identity.phone);
  return identity;
}
  ...
  return Promise.all(
    (calls || []).map(async (task) => {
      ...
      return Object.assign({}, task, {
        ...
        us_state: identity.us_state || "",
        timezone_label: identity.timezone_label || "",
...
function locationEmailSuffix(person) {
  const label = String((person && person.location_label) || "").trim();
  if (!label) return "";
  return ` · ${escapeEmailHtml(label)}`;
}
...
async function fetchManualMorningContactList(supabaseUrl, serviceKey) {
  ...
  await Promise.all([
    loadByIds(
      ...
      quoteIds,
      "id,email,phone,first_name,last_name,state_code,consent_summary,consent_expires_at,payload",
      quoteById
    ),
    loadByIds("manychat_leads", manychatIds, "id,email,phone,first_name,last_name", manychatById),
    loadByIds("contacts", contactIds, "id,email,phone,first_name,last_name,us_state", contactById),
  ]);
  ...
  for (const row of candidates) {
    ...
    const quotePayload = quote && quote.payload && typeof quote.payload === "object" ? quote.payload : {};
    applyLocationFields(
      person,
      stateFromRecord(pd) ||
        stateFromRecord(contact) ||
        (quote && (quote.state_code || quotePayload.state || quotePayload.state_code || quotePayload.us_state)) ||
        "",
      person.phone
    );
...
async function sendDailySummaryEmail(cfg) {
  ...
  (data.manual_contact_list || []).forEach((c) => {
    ...
    lines.push(`<li><strong>${name}</strong>${phone}${locationEmailSuffix(c)}<br>${stage} · ${missing}</li>`);
  });
  ...
  notes.forEach((item) => {
    ...
    lines.push(`<li><strong>${name}</strong>${phone}${locationEmailSuffix(item)} · ${when}<br>${note || "(no note)"}</li>`);
  });
  ...
  (data.new_call_tasks || []).forEach((c) => {
    ...
    lines.push(
      `<li><strong>${name}</strong>${phone}${locationEmailSuffix(c)}<br>${attempt}${day} · due ${when}</li>`
    );
  });
