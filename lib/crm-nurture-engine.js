...
async function loadSettings(supabaseUrl, serviceKey) {
  ...
  try {
    ...
    const cfg =
      rows && rows[0] && rows[0].config && typeof rows[0].config === "object"
        ? Object.assign({}, DEFAULT_CRM_NURTURE_SETTINGS, rows[0].config)
        : Object.assign({}, DEFAULT_CRM_NURTURE_SETTINGS);
    ...
    return Object.assign({}, DEFAULT_CRM_NURTURE_SETTINGS);
  }
}
...
function buildContactedTasks(enrollment, settings) {
  ...
  const callDue = localTimeUtcIso(addDaysYmd(baseYmd, callDays), cs.call_time || "09:30", tz);
  const emailDue = localTimeUtcIso(addDaysYmd(baseYmd, emailDays), cs.email_time || "10:00", tz);
  return [
    {
      task_type: "call",
      due_at: callDue,
      payload: { attempt: 1, recurring: true, interval_days: callDays },
    },
    {
      task_type: "email",
      due_at: emailDue,
      payload: { template: cs.email_template || "contacted_educational", recurring: true, interval_days: emailDays },
    },
  ];
}
...
/** Contacted recurring tasks from phase start through now, plus next future call + email. */
function buildBackdatedContactedTasks(enrollment, settings, asOf) {
  const tz = settings.timezone || "America/Chicago";
  const cs = settings.contacted_sequence || {};
  const callInterval = Number(cs.call_interval_days) || 14;
  ...
  let attempt = 1;
  let callYmd = addDaysYmd(baseYmd, callInterval);
  let callDueMs = new Date(localTimeUtcIso(callYmd, cs.call_time || "09:30", tz)).getTime();
  while (callDueMs <= now.getTime()) {
    tasks.push({
      task_type: "call",
      due_at: new Date(callDueMs).toISOString(),
      payload: { attempt, recurring: true, interval_days: callInterval },
      status: "skipped",
      cancelled_reason: "backdated_past_due",
      completed_at: new Date(callDueMs).toISOString(),
    });
    attempt += 1;
    callYmd = addDaysYmd(callYmd, callInterval);
    callDueMs = new Date(localTimeUtcIso(callYmd, cs.call_time || "09:30", tz)).getTime();
  }
  tasks.push({
    task_type: "call",
    due_at: new Date(callDueMs).toISOString(),
  ...
  while (emailDueMs <= now.getTime()) {
    tasks.push({
      ...
      payload: {
        template: cs.email_template || "contacted_educational",
        recurring: true,
...
function projectFutureContactedTasks(tasks, settings, asOf, extraCycles) {
  ...
  const lastCall = lastRecurring("call");
  if (lastCall) {
    let attempt = (Number(lastCall.payload.attempt) || 1) + 1;
    let ymd = addDaysYmd(ymdChicago(new Date(lastCall.due_at), tz), callInterval);
    for (let i = 0; i < cycles; i++) {
      projected.push({
        task_type: "call",
        due_at: localTimeUtcIso(ymd, cs.call_time || "09:30", tz),
        status: "upcoming",
        payload: {
          attempt,
          recurring: true,
          interval_days: callInterval,
          projected: true,
        },
      });
      attempt += 1;
      ymd = addDaysYmd(ymd, callInterval);
    }
  }
...
async function executeTask(supabaseUrl, serviceKey, task, enrollment, settings, dryRun) {
  ...
  if (task.task_type === "email") {
    ...
    const tpl = getCrmNurtureEmail(payload.template, contact, settings);
    if (!tpl) {
      result.ok = false;
...
async function buildDailySummaryData(cfg) {
  ...
  const callTasks = await sbFetch(
    supabaseUrl,
    serviceKey,
    `/crm_call_tasks?status=eq.pending&due_at=gte.${encodeURIComponent(
      dayStart
    )}&due_at=lte.${encodeURIComponent(dayEnd)}&select=*&order=due_at.asc&limit=200`
  );

  const newCalls = (callTasks || []).filter((c) => c.stage === "new");
  const contactedCalls = (callTasks || []).filter((c) => c.stage === "contacted");

  const pendingComms = await sbFetch(
    supabaseUrl,
    serviceKey,
    `/crm_nurture_tasks?status=eq.pending&due_at=gte.${encodeURIComponent(
      dayStart
    )}&due_at=lte.${encodeURIComponent(dayEnd)}&task_type=in.(email,sms)&select=task_type,due_at,enrollment_id&limit=100`
  );

  return {
    date: todayYmd,
    timezone: tz,
    new_call_tasks: newCalls,
    contacted_call_tasks: contactedCalls,
    pending_emails_sms: pendingComms || [],
    new_call_count: newCalls.length,
    contacted_call_count: contactedCalls.length,
  };
}
...
async function sendDailySummaryEmail(cfg) {
  ...
  const lines = [
    ...
    `<h3>New leads — call tasks today (${data.new_call_count})</h3>`,
    "<ul>",
  ];
  (data.new_call_tasks || []).forEach((c) => {
    lines.push(`<li>Attempt #${c.attempt_number} — due ${new Date(c.due_at).toLocaleString("en-US", { timeZone: data.timezone })}</li>`);
  });
  if (!data.new_call_count) lines.push("<li>None scheduled</li>");
  lines.push("</ul>");

  lines.push(`<h3>Contacted — call tasks today (${data.contacted_call_count})</h3><ul>`);
  (data.contacted_call_tasks || []).forEach((c) => {
    lines.push(`<li>Attempt #${c.attempt_number} — due ${new Date(c.due_at).toLocaleString("en-US", { timeZone: data.timezone })}</li>`);
  });
  if (!data.contacted_call_count) lines.push("<li>None scheduled</li>");
  lines.push("</ul>");

  lines.push(`<h3>Emails / SMS queued today (${(data.pending_emails_sms || []).length})</h3>`);
