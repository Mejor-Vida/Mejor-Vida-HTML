...
function mergeTruthyFields(fields) {
  const out = {};
...
async function getContactByPhone(supabaseUrl, serviceKey, phone) {
  const clean = cleanField(phone);
  if (!clean) return null;
  const exactUrl = `${base(supabaseUrl)}/contacts?phone=eq.${encodeURIComponent(clean)}&limit=1`;
  const exactRes = await fetch(exactUrl, { headers: restHeaders(serviceKey) });
  const exactText = await exactRes.text();
  if (!exactRes.ok) throw new Error(`contacts select ${exactRes.status}: ${exactText.slice(0, 300)}`);
  const exactRows = JSON.parse(exactText);
  if (exactRows.length > 0) return exactRows[0];

  const digits = clean.replace(/\D+/g, "");
  const last10 = digits.length >= 10 ? digits.slice(-10) : "";
  if (!last10) return null;
  const lastUrl = `${base(supabaseUrl)}/contacts?phone_last_10=eq.${encodeURIComponent(last10)}&limit=1`;
  const lastRes = await fetch(lastUrl, { headers: restHeaders(serviceKey) });
  const lastText = await lastRes.text();
  if (!lastRes.ok) {
    if (/phone_last_10|42703/i.test(lastText)) return null;
    throw new Error(`contacts select last10 ${lastRes.status}: ${lastText.slice(0, 300)}`);
  }
  const lastRows = JSON.parse(lastText);
  return lastRows.length > 0 ? lastRows[0] : null;
}

/**
 * Find a contact by ManyChat subscriber id (whatsapp_id or manychat_subscriber_id column).
 */
async function getContactByManychatSubscriberId(supabaseUrl, serviceKey, subscriberId) {
  const sid = encodeURIComponent(String(subscriberId || "").trim());
  if (!sid) return null;
  const url = `${base(supabaseUrl)}/contacts?or=(whatsapp_id.eq.${sid},manychat_subscriber_id.eq.${sid})&limit=1`;
  const r = await fetch(url, { headers: restHeaders(serviceKey) });
  const text = await r.text();
  if (!r.ok) throw new Error(`contacts select by subscriber ${r.status}: ${text.slice(0, 300)}`);
  const rows = JSON.parse(text);
  return rows.length > 0 ? rows[0] : null;
}

async function getContactsByManychatSubscriberId(supabaseUrl, serviceKey, subscriberId) {
  const sid = encodeURIComponent(String(subscriberId || "").trim());
  if (!sid) return [];
  const url = `${base(supabaseUrl)}/contacts?or=(whatsapp_id.eq.${sid},manychat_subscriber_id.eq.${sid})&select=*&order=created_at.desc&limit=50`;
  const r = await fetch(url, { headers: restHeaders(serviceKey) });
  const text = await r.text();
  if (!r.ok) throw new Error(`contacts select by subscriber ${r.status}: ${text.slice(0, 300)}`);
  const rows = JSON.parse(text);
...
/**
 * Find a contact by primary key. Returns full row or null.
 */
async function getContactById(supabaseUrl, serviceKey, contactId) {
  const url = `${base(supabaseUrl)}/contacts?id=eq.${encodeURIComponent(contactId)}&limit=1`;
  const r = await fetch(url, { headers: restHeaders(serviceKey) });
  const text = await r.text();
...
async function upsertContact(supabaseUrl, serviceKey, phone, fields) {
  ...
  const sanitized = mergeTruthyFields(fields || {});
  const subscriberId = cleanField(sanitized.whatsapp_id || sanitized.manychat_subscriber_id);

  // Canonical preference: exact phone row is source of truth.
  const existingByPhone = cleanPhone ? await getContactByPhone(supabaseUrl, serviceKey, cleanPhone) : null;
  const candidatesBySubscriber = subscriberId
    ? await getContactsByManychatSubscriberId(supabaseUrl, serviceKey, subscriberId)
    : [];
  const existingBySubscriber = candidatesBySubscriber.find((c) => cleanField(c.phone) === cleanPhone) || candidatesBySubscriber[0] || null;
  const target = existingByPhone || existingBySubscriber;

  if (target) {
    const updatePayload = { ...sanitized };
    // If we matched by subscriber id only, normalize template/legacy phone to the stable incoming WhatsApp phone.
    if (!existingByPhone && cleanPhone && cleanField(target.phone) !== cleanPhone) {
      updatePayload.phone = cleanPhone;
    }
    if (sanitized.created_at) {
      const keep = earlierIso(sanitized.created_at, target.created_at);
      if (keep && String(keep) !== String(target.created_at || "")) {
        updatePayload.created_at = keep;
      } else {
    ...
    await updateContact(supabaseUrl, serviceKey, target.id, updatePayload);
    return { contactId: target.id, created: false };
  }
  const insertFields = { phone: cleanPhone, ...sanitized };
  if (sanitized.created_at) insertFields.created_at = sanitized.created_at;
  const row = await insertContact(supabaseUrl, serviceKey, insertFields);
...
module.exports = {
  // contacts
  getContactByPhone,
  getContactByManychatSubscriberId,
