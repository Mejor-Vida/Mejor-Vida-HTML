...
function filterSessions(sessions, filters) {
  ...
  sessions.forEach((s) => {
    ...
    if (viewId === "lp_direct" && !onLanding) return;
    if (viewId === "website" && onLanding) return;
    if (view.landingVariant) {
      if (!onLanding) return;
      if (sessionLandingVariant(s) !== view.landingVariant) return;
      if (!matchesFacebookAbTestSession(s, view.landingVariant)) return;
    }
