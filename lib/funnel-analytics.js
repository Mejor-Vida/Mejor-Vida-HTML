...
function filterSessions(sessions, filters) {
  ...
  sessions.forEach((s) => {
    ...
    if (landingPage === "website") {
      ...
      if (!onLanding) return;
      if (!hasMeaningfulAdAttribution(acq)) return;
...
function sessionReachedStep(session, tool, step) {
  return (session.events || []).some((ev) => {
    if (!eventMatchesStep(ev, step)) return false;
    if (step.match && step.match.tool) return true;
    return ev.tool === tool;
  });
}
...
function buildFunnelBranches(sessions, viewId) {
  ...
  const order = getBranchOrder(viewId);
  const out = {};
...
function applyWhatsappAdFunnel(dashboard, stats) {
  ...
  const scheduledCalls = Number(stats && stats.scheduledCalls) || 0;
  const counts = [clicks, conversations, quoted, scheduledCalls];
  const labels = ["Ad clicks", "Conversations started", "Quote in chat", "Scheduled-call leads"];
  const ids = ["ad_clicks", "conversations", "quoted", "scheduled_calls"];
  ...
  dashboard.branchOrder = ["whatsapp"];
  return dashboard;
}
...
function buildDashboard(events, filters) {
  ...
  return {
    ...
    branchOrder: getBranchOrder(viewId),
