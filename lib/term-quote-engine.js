...
function cheapestInPool(rows, allowedClasses, faceAmount) {
  ...
  for (const row of rows) {
    ...
    if (best == null || monthly < best) {
      ...
      bestProduct = row.product;
    }
  }
  return best == null
    ? null
    : { monthly: best, carrier: bestCarrier, product: bestProduct };
}
...
function computeTermQuoteRange(params, rateRows) {
  ...
  const cheapest = (list) =>
    list.reduce((acc, x) => (acc == null || x.monthly < acc.monthly ? x : acc), null);
  const lowWinner = cheapest(lowCandidates);
  const highWinner = cheapest(highCandidates);

  let low = lowWinner ? lowWinner.monthly : null;
  let high = highWinner ? highWinner.monthly : null;
  ...
  return {
    ...
    lowCarrier: (lowWinner || highWinner || {}).carrier || null,
    lowProduct: (lowWinner || highWinner || {}).product || null,
  };
}
