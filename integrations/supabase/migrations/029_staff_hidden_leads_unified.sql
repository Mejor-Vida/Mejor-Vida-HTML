-- Staff compose: hide leads from unified cross-source directory.
-- Keeps source tables untouched; stores suppression keys for unified_leads filtering.

CREATE TABLE IF NOT EXISTS public.staff_hidden_leads (
...
CREATE INDEX IF NOT EXISTS idx_staff_hidden_leads_hidden_at
  ON public.staff_hidden_leads (hidden_at DESC);

CREATE INDEX IF NOT EXISTS idx_staff_hidden_leads_source
  ON public.staff_hidden_leads (source_table, source_id);

COMMENT ON TABLE public.staff_hidden_leads IS
  'Cross-source suppression list for staff compose unified_leads dropdown.';

ALTER TABLE public.staff_hidden_leads ENABLE ROW LEVEL SECURITY;
...
filtered AS (
  SELECT d.*
  FROM deduped d
  LEFT JOIN public.staff_hidden_leads h ON h.dedupe_key = d.dedupe_key
  WHERE d.rn = 1
    AND h.id IS NULL
)
SELECT
...
COMMENT ON VIEW public.unified_leads IS
  'Read-only lead directory for staff compose. Dedupes lead sources and excludes staff_hidden_leads suppressions.';
