-- Staff compose: match hidden leads by source row, not by unified partition dedupe_key.
-- Previously staff_hidden_leads.dedupe_key was often a bare phone digit string, matching
-- unified_leads.dedupe_key for the whole partition and hiding every future lead on that phone.

UPDATE public.staff_hidden_leads
SET dedupe_key = COALESCE(BTRIM(source_table), 'unknown') || ':' || source_id::text
WHERE source_id IS NOT NULL
  AND BTRIM(COALESCE(source_table, '')) <> '';
...
COMMENT ON VIEW public.unified_leads IS
  'Read-only lead directory for staff compose. Dedupes lead sources; staff_hidden_leads suppresses by source row when source_table+source_id are set.';
