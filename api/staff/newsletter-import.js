/**
 * POST /api/staff/newsletter-import — import weekly newsletter content.
 */
...
module.exports = async function handler(req, res) {
  ...
  const heroHtml = String(body.hero_html || body.heroHtml || "").trim();
  const bodyHtml = String(body.body_html || body.bodyHtml || "").trim();
  const subject = String(body.subject || "").trim() || "Mejor Vida Insurance — Weekly Update";
  const heroSource = String(body.hero_source || body.heroSource || "import").trim();
  const blogUrl = String(body.blog_url || body.blogUrl || "").trim() || null;

  if (!heroHtml && !bodyHtml) {
    return json(res, 400, { error: "hero_html or body_html required" });
  }
