...
**First comment (link + WhatsApp):** After publish, `facebook-posting/main.py` posts the **first comment immediately** via the Meta Graph API using `first_comment` from `post-package.json`. Target **~8 lines**; phone number only (no long `wa.me` in the comment). Legacy Make.com path: `--first-comment-make` (see **`integrations/MAKE_COM_FB_FIRST_COMMENT.md`**).
