...
**Comment replies** (`lib/facebook-comment-reply.js` — this is the source of truth):

- **First reply** to a person: thank them; say you are the automated assistant of Mejor Vida Seguros; Julie has been notified and will send a **private message** ASAP; then one simple public answer if they asked a real question.
- **Later replies** in that conversation: skip the intro. One short **usted** answer (1–2 sentences). End with “Si tiene otra pregunta, puede escribirla aquí.”
- Bare **INFO** / “information” / **REVISAR** as a keyword: intro only — **no** weekly blog article, no URL dump.
- Real questions pull from **RAG**, rewritten as public-facing copy (no website-chat phone/email dumps). Fallback canned lines only if RAG is empty.
