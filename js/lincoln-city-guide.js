/**
 * Lincoln city guide: live-ish plot resale board + one-page funeral/coverage calculator.
 * Resale cards read data/lincoln-plot-resales.json so listings can be updated
...
(function () {
  ...
  async function loadResales(root) {
    var urls = [root.getAttribute("data-resale-src"), "/data/lincoln-plot-resales.json"].filter(Boolean);
    var lastErr = null;
  ...
  function initResales() {
    var root = document.getElementById("lincoln-resale-board");
    if (!root) return;
    loadResales(root)
      .then(function (data) {
  ...
  var GPL = {
    alt: {
      directCremation: 1595,
      memorialCremation: 2595,
      immediateBurial: 1805,
      traditional: 3155,
    },
    lm: {
      directCremation: 3910,
      memorialCremation: 5795,
      immediateBurial: 4920,
      traditional: 11935,
    },
    wyuka: {
      directCremation: 3285,
      memorialCremation: 3960,
      immediateBurial: 3195,
      traditional: 5800,
    },
    us: {
      directCremation: 2958,
      memorialCremation: 6530,
  ...
  function homeIncludesCasket(home, service) {
    if (home === "lm") return service === "traditional" || service === "memorialCremation";
    if (home === "us") return service !== "directCremation";
    return false;
  }

  var PLOT_NEW = 4000;
  var PLOT_RESALE = 2000;
  var VAULT = 1495;
  var CASKET_TYPICAL = 2500;
  ...
  function initCalc() {
    var form = document.getElementById("lincoln-fe-calc");
    if (!form) return;
    ...
    function homeLabel(home) {
      return {
        alt: t("Alternative Funeral", "Alternative Funeral"),
        lm: t("Lincoln Memorial", "Lincoln Memorial"),
        wyuka: "Wyuka",
        us: t("Estimator (Nebraska)", "Estimador (Nebraska)"),
      }[home];
    }
    ...
    async function lookupQuote(coverage) {
      ...
      setPremium(
        ...
        "",
        t(
          "Could not load appointed-company rates. Use Get a real quote, or call 402-440-5438.",
          "No se pudieron cargar las tarifas. Pida una cotización real o llame al 402-440-5438."
        )
      );
    }
